registerShadertoy('Inside the Mandelbulb II', `
// CC0: Inside the mandelbulb II
// Single-pass version combining rendering and anti-aliasing
...
// Simple AA by supersampling
vec3 effectAA(vec2 p, vec2 pixel) {
  vec3 col = vec3(0.0);
  
  // 2x2 supersampling for basic AA
  const float AA = 2.0;
  for(float i = 0.0; i < AA; i++) {
    for(float j = 0.0; j < AA; j++) {
      vec2 offset = vec2(i, j) / AA - 0.5;
      vec2 pp = p + offset * pixel;
      col += effect(pp);
    }
  }
  col /= (AA * AA);
