...
registerShadertoy('Shadertoy Test - Brick Corridor', `
    ...
    */
    ...
    vec3 tex3D( in vec3 p, in vec3 n ){
        ...
        // Procedural brick-like pattern
        vec3 col1 = vec3(0.5 + 0.5 * sin(p.y * 10.0) * sin(p.z * 10.0));
        vec3 col2 = vec3(0.5 + 0.5 * sin(p.z * 10.0) * sin(p.x * 10.0));
        vec3 col3 = vec3(0.5 + 0.5 * sin(p.x * 10.0) * sin(p.y * 10.0));
        
        p = (col1*n.x + col2*n.y + col3*n.z);
        return p*p;
    }
    ...
    */
    ...
    vec3 doBumpMapTex( in vec3 p, in vec3 n, float bf){
    ...
    void mainImage( out vec4 fragColor, in vec2 fragCoord ){
        ...
        if(t<FAR){
            ...
            sn = doBumpMapTex(sp*tSize0, sn, 0.02);
