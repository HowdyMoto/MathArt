registerShadertoy('Morphing Julia Sets', `
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    float zoom = 1.5 + 0.5 * sin(iTime * 0.2);
    ...
    c += vec2(sin(iTime * 0.7) * 0.02, cos(iTime * 0.9) * 0.02);
    ...
    if(iter < 128.0) {
        ...
        float hue1 = mod(hue_base + iTime * 0.1, 1.0);
        float hue2 = mod(hue_base + iTime * 0.05 + 0.3, 1.0);
        ...
        float mix_factor = sin(smooth_iter * 0.3 + iTime) * 0.5 + 0.5;
        color = mix(color1, color2, mix_factor);
        
        // Add brightness modulation for more dynamic appearance
        float brightness_mod = 0.8 + 0.2 * sin(iTime * 2.0 + smooth_iter * 0.1);
