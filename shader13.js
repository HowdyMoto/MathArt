...
registerShadertoy('Organic Shader', `
...
// Define AA (anti-aliasing) level - reduce to 1 if performance is poor
#define AA 1
...
vec4 inverseSF( in vec3 p, const in float n )
{
    ...
    for( int s=0; s<4; s++ )
    {
        vec2 uv = vec2(mod(float(s),2.0),floor(float(s)/2.0));
        float i = dot(F,uv+c);
...
vec3 calcNormal( in vec3 pos, in float ep )
{
    ...
    {
        float fi = float(i);
        vec3 e = 0.5773*(2.0*vec3(mod(floor((fi+3.0)/2.0),2.0),mod(floor(fi/2.0),2.0),mod(fi,2.0))-1.0);
